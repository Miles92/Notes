...
void OnTick()
{
    ...
    // Comment the rounded spread, RSI value, and server time on the graph
    ...
    if (MYRSIValue > RSI_ShortThreshold && !IsTradeOpen(ORDER_TYPE_SELL) && !shortTradeOpened && (tradeMode == TradeMode::LongAndShort || tradeMode == TradeMode::OnlyShort))
    {
        // Enter a short trade
        EnterShortTrade();
    ...
    else if (MYRSIValue < RSI_LongThreshold && !IsTradeOpen(ORDER_TYPE_BUY) && !longTradeOpened && (tradeMode == TradeMode::LongAndShort || tradeMode == TradeMode::OnlyLong))
    {
        // Enter a long trade
        EnterLongTrade();
